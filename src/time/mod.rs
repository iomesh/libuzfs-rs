...
use driver::{duration_from_now, Timer, TimerManager};
use libc::timespec;
...
pub fn sleep(duration: timespec) -> Sleep {
    let expiration = duration_from_now(&duration);
    Sleep {
        timer: TIMER_MANAGER.get().unwrap().new_timer(expiration),
        registered: false,
        finished: false,
    }
}
...
pub fn timeout<F: Future>(duration: timespec, fut: F) -> Timeout<F> {
    Timeout {
        fut,
