...
async fn uzfs_snapshot_test() {
    ...
    let mut ino_hdl = ds.create_inode(InodeType::DIR).await.unwrap();
    let (ino, gen) = (ino_hdl.ino, ino_hdl.gen);
    let snap1 = "snap1";
    Dataset::create_snapshot(dsname, snap1).await.unwrap();
    ds.delete_inode(&mut ino_hdl, InodeType::DIR).await.unwrap();
    ds.release_inode_handle(&mut ino_hdl).await;
    ds.get_inode_handle(ino, gen, false).await.unwrap_err();

    let ds_snap1 = Dataset::open_snapshot(dsname, snap1).await.unwrap();
    let mut ino_hdl = ds_snap1.get_inode_handle(ino, gen, false).await.unwrap();
    ds_snap1.release_inode_handle(&mut ino_hdl).await;
    ds_snap1.close().await.unwrap();
    ds.rollback(snap1).await.unwrap();
    let mut ino_hdl = ds.get_inode_handle(ino, gen, false).await.unwrap();
    let mut ino_hdl2 = ds.get_inode_handle(ino, gen, false).await.unwrap();
    assert_eq!(ino_hdl, ino_hdl2);
    ds.release_inode_handle(&mut ino_hdl2).await;
