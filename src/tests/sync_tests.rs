...
async fn mutex_test() {
    ...
    for _ in 0..10000 {
        ...
        for _ in 0..concurrency {
            let coroutine = CoroutineFuture::new(mutex_worker, &mut value as *mut _ as usize);
...
async fn rwlock_test() {
    ...
    for _ in 0..100 {
        ...
        let mut handles = Vec::with_capacity(concurrency);
        for _ in 0..concurrency {
            let coroutine = CoroutineFuture::new(rwlock_writer, &mut value as *mut _ as usize);
            handles.push(tokio::spawn(coroutine));

            let coroutine = CoroutineFuture::new(rwlock_reader, &mut value as *mut _ as usize);
            handles.push(tokio::spawn(coroutine));
        }
...
async fn block_queue_test() {
    ...
    for _ in 0..workers {
        let coroutine = CoroutineFuture::new(block_queue_producer, &mut bq_test as *mut _ as usize);
        handles.push(tokio::spawn(coroutine));

        let coroutine = CoroutineFuture::new(block_queue_consumer, &mut bq_test as *mut _ as usize);
        handles.push(tokio::spawn(coroutine));
    }
