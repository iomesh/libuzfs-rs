...
impl Record {
    fn decode(mut buffer: &[u8]) -> Result<Option<Self>> {
        ...
        buffer = &buffer[8..];
        let generation = u64::from_be_bytes(buffer[..8].try_into().unwrap());
        buffer = &buffer[8..];
    ...
    fn encode_to(&self, mut buffer: &mut [u8]) {
        ...
        buffer[offset..offset + 8].copy_from_slice(&self.generation.to_be_bytes());
        offset += 8;
...
impl Checker {
    ...
    async fn check(mut self) -> Result<Option<Writer>> {
        ...
        let last_gen = record.map(|r| r.generation).unwrap_or(0);
...
impl Writer {
    async fn write_record(&mut self, active: bool) -> Result<()> {
        ...
        let mut buffer = AlignedScsiBuffer::new(lbs * 2);
        let record = Record {
            hostid: self.hostid.clone(),
            generation: self.next_gen - 1,
            active: true,
        };
        record.encode_to(buffer.slice_mut_from(0, lbs));
