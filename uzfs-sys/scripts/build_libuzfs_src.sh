...
ZFS_PKG=zfs-uzfs-dev-minitrace
ZFS_ZIP=${ZFS_PKG}.zip
INSTALL_DIR=${TOP_SRCDIR}/install
LIBUZFS=${INSTALL_DIR}/lib/libuzfs.a
DOWNLOAD_DIR=${TOP_SRCDIR}/download
DOWNLOAD_URL=https://github.com/iomesh/zfs/archive/refs/heads/uzfs-dev-minitrace.zip
...
build_libuzfs_lib() {
    cd ${ZFS_DIR}
    ./autogen.sh && CFLAGS=-fPIC ./configure --with-config=user --with-minitrace_c=no --enable-shared=no --enable-debuginfo=${ENABLE_DEBUG} --enable-debug=${ENABLE_DEBUG} --prefix=${INSTALL_DIR} && make gitrev
    cd lib
